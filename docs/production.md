...
- 全部写请求串行处理，排队后再次校验浏览器绑定的知识库。正文使用 SHA-256 版本校验，过期保存返回 409 并保留浏览器输入。写入采用同目录临时文件、刷盘、原子替换；Windows 使用替换现有文件的系统接口，遇到短暂文件占用时有限重试，重试前重新核对正文版本；持续占用超过重试预算时保留原文件并报告失败，释放占用后可重新保存。与飞书共享写锁，防止同步期间写入。
