...
打开笔记后，左侧自动切换到「大纲」，按正文的 1–6 级标题缩进显示；点击标题跳转到相应章节，滚动正文时高亮当前章节。标题修改后实时更新，同名标题也可分别定位。点击「文件」切回目录树；搜索或筛选标签时自动显示文件列表。手机上可通过顶部展开目录按钮打开左栏，再点击大纲标题回到正文。没有标题时显示添加标题的提示，代码块中的 `#` 不会被当作标题；原文编辑模式下支持定位并选中标题。
